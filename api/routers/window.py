...
def get_ticket(session: Session, department_id: str, services):
    ...
    statement = select(QueueItem).where(
        ...
        col(QueueItem.creation_time).asc()
    ).limit(1)
    ...
    with Session(engine) as session:
        ...
        if ticket:
            ...
            try:
                ...
                session.rollback()
                raise HTTPException(status_code=400, detail=f"Error request new: {str(e)}")
